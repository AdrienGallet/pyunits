...
unitDict = {
	...
	'length': [['μm', 'mm', 'cm', 'dm', 'm', 'km'], [1e6, 1e3, 1e2, 1e1, 1, 1e-3], 'm', [1,0,0,0,0,0,0]],
	'area': [['μm2', 'mm2', 'cm2', 'dm2', 'm2', 'km2'], [1e12, 1e6, 1e4, 1e2, 1, 1e-6], 'm2', [2,0,0,0,0,0,0]],
	'section mod.|volume': [['μm3', 'mm3', 'cm3', 'dm3', 'm3', 'km3'], [1e18, 1e9, 1e6, 1e3, 1, 1e-9], 'm3', [3,0,0,0,0,0,0]],
	'mom. of inert.|tors. const.': [['μm4', 'mm4', 'cm4', 'dm4', 'm4', 'km4'], [1e24, 1e12, 1e8, 1e4, 1, 1e-12], 'm4', [4,0,0,0,0,0,0]],
	'wraping constant': [['μm6', 'mm6', 'cm6', 'dm6', 'm6', 'km6'], [1e36, 1e18, 1e12, 1e6, 1, 1e-18], 'm6', [6,0,0,0,0,0,0]],
	...
	'stress|strain': [['Pa', 'kPa', 'MPa', 'N/mm2'], [1, 1e-3, 1e-6, 1e-6], 'Pa', [-1,1,-2,0,0,0,0]]
			}
...
class phyVar():
	...
	def __init__(self, name, value=None, unit=None, SIID=None, info=None, formula=None):
		...
		if (name!=None and value!=None and unit!=None):
		...
		elif (value==None and unit==None): #Check if name string contains input variables
			...
			if len(nameSplit) < 2: #Missing equal sign
				...
				raise Exception('phyVar() declaration has more than one equal [=] sign.')
			else: #Continue to extract phyVar
				self.name = nameSplit[0] #Retrieve the name
				tmpOut = re.search(r'[^0-9|^.]', nameSplit[1]) #Evaluate position of first unit character
				value, unit = nameSplit[1][:tmpOut.start()], nameSplit[1][tmpOut.start():] #Split the string at that location
			...
			raise Exception('missing input, need to specify variable name, value and unit. Ex: phyVar(\'a=10m\') or phyVar(\'a\', \'10\', \'m\')).')
		else:
		...
		if len(lstUnitSet) == 0: #Unknown unitSet
			...
			if self.SIID == None:
				raise Exception('unknown units, SIID must be specified within phyVar call.')
			...
			if self.unitBase == self.unit: self.valueBase = self.value
			else: self.valueBase = round(self.convert(self.unitBase, out='value'), precLevel)

	#--- A dunder method to change what is shown with print()
	def __repr__(self):
		rString = 'phyVar(\'{}\', value={}, unit=\'{}\', SIID={}, info={}, formula={})'.format(self.name, self.value, self.unit, self.SIID, self.info, self.formula)
		return rString
	...
	def __add__(self, other):
		...
		if not isinstance(other, phyVar):
			...
			if newPhyVar.unitBase == self.unit:
				...
				newPhyVar.valueBase = round(self.valueBase + phyVar('tmp', other, self.unit).convert(self.unitBase, out='value'), precLevel)
		...
		if self.SIID != other.SIID:
			raise Exception('cannot add/subtract values from different unitSets. Unit sets in question: {} and {} | SIID: {} and {}.'.format(self.unitSet, other.unitSet, self.SIID, other.SIID))
		...
		if len(lstUnitSet) == 0: #unit does not exist within unitSets
			return phyVar(tmpName, round(self.valueBase + other.valueBase, precLevel), self.unitBase, SIID=self.SIID, formula=tmpName) #Return new phyVar
		else: #List has length 1, meaning unitSet exists
			return phyVar(tmpName, round(self.valueBase + other.valueBase, precLevel), self.unitBase, formula=tmpName) #Return new phyVar
	...
	def __sub__(self, other): #Does negated addition
		...
		if not isinstance(other, phyVar): return self.__add__(-other)
		...
		#If it is a phyVar, make the appropriate changes to the "other" phyVar, and then perform addition of it's negative value
			...
			other.value *= -1; other.valueBase *= -1 #Negate values
			...
			newPhyVar.name = tmpName; newPhyVar.formula=tmpName
	...
	def __mul__(self,other):
		...
		if not isinstance(other, phyVar):
			...
			newPhyVar.valueBase = round(self.valueBase * other, precLevel) #Update the SI value (no conversion needed)
			return newPhyVar #Return the object itself
		...
		if len(lstUnitSet) == 0: #unitSet does not exist
			tmpUnit = phyVar.findUnitFromSIID(newSIID) #Find the correct unit from SIID

			#Conduct operation
			return phyVar(tmpName, round(self.valueBase * other.valueBase, precLevel), tmpUnit, SIID=newSIID, formula=tmpName)
		...
		#Only other option is that len(lstUnitSet) == 1, the unitSet exists
			...
			#Conduct operation
			return phyVar(tmpName, round(self.valueBase * other.valueBase, precLevel), tmpUnit, formula=tmpName)

	#--- Division magic, method for arithmetics
	def __truediv__(self,other):
		#If "other" is not a phyVar (assuming it is a number)
		if not isinstance(other, phyVar): return self.__mul__(1/other)
		...
		#If it is a phyVar, make the appropriate changes to the "other" phyVar
			...
			#Then conduct operation
			other.value = 1/other.value; other.valueBase = 1/other.valueBase #Invert values
	...
	def __rtruediv__(self, other): #Needs adjustment b/c division is not commutative
		...
		newSelf.value = 1/newSelf.value; newSelf.valueBase = 1/newSelf.valueBase
	...
	def __neg__(self):
		...
		newPhyVar.value *= -1; newPhyVar.valueBase *= -1
		return newPhyVar

	#--- Unary arithemtic, absolute
	def __abs__(self):
		newPhyVar = copy.deepcopy(self)
		newPhyVar.value = abs(newPhyVar.value); newPhyVar.valueBase = abs(newPhyVar.valueBase)
		return newPhyVar
	...
	def convert(self, toUnit, out='variable'):
		...
		except ValueError: raise Exception('unit to convert to does not exist within the unitSet.')

		#Check also that fromUnit is still within the unitSet
		try:
			fromUnitIndex = unitDict[self.unitSet][0].index(self.unit)
		except ValueError: raise Exception('unit to convert from does not exist within the unitSet.')

		#Evaluate the conversion factor
		conversionFactor = unitDict[self.unitSet][1][toUnitIndex] / unitDict[self.unitSet][1][fromUnitIndex]
	...
	def stringVar(self):
		return '{} = {}{}, unitSet: {}, valueBase: {}, unitBase: {}, info: {}, formula: {}, SIID: {}'.format(self.name, self.value, self.unit, self.unitSet, self.valueBase, self.unitBase, self.info, self.formula, self.SIID)
	...
	def findUnitSet(unit): #unit can be string name or SIID list
		...
		if isinstance(unit, str):
			return [unitSet for unitSet, unitLists in unitDict.items() if unit in unitLists[0]] #unitLists[0] contains the units as a list of strings
		elif isinstance(unit, list):
			return [unitSet for unitSet, unitLists in unitDict.items() if unit == unitLists[3]] #unitLists[3] contains the SSID as a list
		else:
			raise Exception('findUnitSet retrieved input which was neither a string (unit name) nor a list (SIID list).')

	#Establish the unit based on the SIID for non-unitSet numbers.
	def findUnitFromSIID(SIID):
		tmpUnit = '' #string for the actual units
		lstSIUnits = ['m', 'kg', 's', 'K', 'A', 'mole', 'cd'] #list of SI units, in the order of the SIID list
		...
		for unit, power in zip(lstSIUnits, SIID):
			...
			else: continue #When unit is dimensionless
	...
	def checkSameSIIDCompare(firstVar, secondVar):
		if firstVar.SIID != secondVar.SIID:
			raise Exception('cannot compare values from different unitSets. Unit sets in question: {} and {} | SIID: {} and {}.'.format(firstVar.unitSet, secondVar.unitSet, firstVar.SIID, secondVar.SIID))
...
def unitDicTest():
	...
	if len(set(lstUnits)) != len(lstUnits):
		raise Exception('duplicate units exist either within a unitSet or accross unitsets.')
